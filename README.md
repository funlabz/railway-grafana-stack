...
#### Embedded Alerting
Grafana now provisions unified alerting resources from `grafana/alerting`. Update `alerts.yml`, `contact-points.yml`, and `notification-policies.yml`, then rebuild the Grafana image (`docker compose up --build grafana`) to version alert rules, contact points, and routing with the rest of the stack. The bundled `Polybot Flow Event Detected` rule queries the `FlowEvent` Postgres table for warning/error/critical entries during the last five minutes (using Grafana's SQL macros so the query runs as a time-series). Any non-zero count keeps the series alive, generates a `severity=warning` alert, and lands in Slack; tune the SQL `WHERE` clause or evaluation window to match the exact flows, stages, or severity levels you care about.
